...
def Backtracking(alpha,beta,_a,_x):
    ...
    t=1
    _grad=gradient(a,x)
    reverse_search_direction=(np.linalg.norm(_grad,1))*np.sign(_grad)
    min_err=0.005  #迭代结束时的最小误差
    time=0  #从第0次迭代开始

    #在过程中收集以下参数
    objvalue=[(f(a,x)-p)**2]  #迭代过程中的目标函数值
    steplength=[t]  #迭代过程中的补偿
    grad=[np.linalg.norm(_grad)]  #迭代过程中的梯度二范数
    
    while np.linalg.norm(_grad)>min_err and time<1000:
        time+=1
        t=1
        ...
        steplength.append(t)
        grad.append(np.linalg.norm(_grad))

    #画图
    times=[i for i in range(0,time+1)]
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 显示汉字
    plt.title('最速下降+Backtracking:alpha={},beta={}'.format(alpha,beta))  # 折线图标题
    plt.xlabel('迭代次数')
...
Backtracking(0.01,0.5,a,x)   #a的值必须选取得比较小
